...
MAIN_OWNER = 7961331792
SUPER_ADMIN_NAME = "@GhostX_009z"
SUPER_ADMIN_LINK = "https://t.me/GhostX_009z"
SUPER_ADMINS = [7892250903]

BOT_TOKEN = "8637020985:AAGhort8U7JOsgpXGnkddpJzJWE9d5I7juc"
LOG_CHANNEL_ID = -1003882047668

_DATA_FILE = "govyx_data.json"
_VERSION = "GOVYX v1.0"
...
def owner_panel_text(d: dict) -> str:
    ...
    return (
        f"{em(EMOJI_CROWN, '👑')} <b>{sc('owner panel')}</b> — Gᴏᴠʏx Sᴍs Bᴏᴛ {_VERSION}\n"
...
def user_home_text(uid: int, d: dict) -> str:
    ...
    scan_info = get_scan_status()
    return (
        f"{em(EMOJI_PHONE, '📱')} <b>Gᴏᴠʏx Sᴍs Bᴏᴛ {_VERSION}</b>\n"
        f"<b>Owner:</b> {SUPER_ADMIN_NAME}\n\n"
...
async def user_info(cq: CallbackQuery, state: FSMContext):
    await cq.message.edit_text(
        f"{em(EMOJI_GEAR, 'ℹ️')} <b>Gᴏᴠʏx Sᴍs Bᴏᴛ {_VERSION}</b>\n\n"
...
async def main():
    ...
    dp.include_router(R)
    me = await bot.get_me()
    log.info(f"@{me.username} — Gᴏᴠʏx Sᴍs Bᴏᴛ {_VERSION} started!")
    ...
    try:
        await bot.send_message(
            MAIN_OWNER,
            f"{em(EMOJI_ROCKET, '🚀')} <b>Gᴏᴠʏx Sᴍs Bᴏᴛ  {_VERSION} Online!</b>\n@{me.username}\n"
